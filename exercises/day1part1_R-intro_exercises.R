...
t <- sort(table(gapminder07$continent))
...
agg <- aggregate(gapminder07['lifeExp'], by = list(gapminder07$continent), mean)
...
write.csv(gapminder07, file = "gapminder07.csv",  row.names = FALSE)
...
hist(gapminder07$lifeExp, main = "lifeExp", xlab = "lifeExp", breaks = 5)
...
plot(lifeExp ~ gdpPercap, data = gapminder07, main = "plot")
